...
RSpec.describe OrderSendingInformation, type: :model do
  describe '商品購入' do
    before do
      ...
      @order_sending_information = FactoryBot.build(:order_sending_information, item_id: item.id, user_id: user.id)
    end
    ...
    context '内容に問題がある場合' do
      ...
      it 'postal_codeが半角のハイフンを含んだ正しい形式でないと保存できないこと' do
        @order_sending_information.postal_code = '1234567'
        @order_sending_information.valid?
        expect(@order_sending_information.errors.full_messages).to include("Postal code is invalid. Include hyphen(-)")
      end
      ...
      it 'userが紐付いていないと保存できないこと' do
        ...
        @order_sending_information.valid?
        expect(@order_sending_information.errors.full_messages).to include("User can't be blank")
      end
      it 'itemが紐付いていないと保存できないこと' do
         @order_sending_information.item_id = nil
         @order_sending_information.valid?
        expect(@order_sending_information.errors.full_messages).to include("Item can't be blank")
      end
